...
type TUseSlice<State, T extends CaseReducerActions<any>, K extends Record<string, AsyncThunk<any, any, any>>> = State & ReducerStateActions<keyof State> & Action<T> & PromiseAction<K>;

export const createUseSlice = <State, CaseReducers extends SliceCaseReducers<State> & Reducer<State>, Name extends string = string, Promises extends Record<string, AsyncThunk<any, any, any>> = {}>(
  ...
  return (): TUseSlice<State, typeof actions, typeof promises> => {
    ...
    Object.entries(promises).forEach(([promiseActionKey, promiseActionFunc]) => {
